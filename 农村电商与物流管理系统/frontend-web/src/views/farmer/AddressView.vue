<template>
  <div class="address-view">
    <div class="address-container">
      <h2 class="page-title">发货地址管理</h2>

      <div class="address-list">
        <div v-for="addr in addresses" :key="addr.id" class="address-card">
          <div class="address-content">
            <div class="address-header">
              <span class="address-name">{{ addr.senderName }}</span>
              <span class="address-phone">{{ addr.senderPhone }}</span>
      ...
      <!-- 空状态 -->
      <el-empty v-if="addresses.length === 0 && !loading" description="暂无发货地址，请添加新地址" />
    </div>
    ...
    >
      <el-form :model="addressForm" label-width="80px">
        <el-form-item label="发货人">
          <el-input v-model="addressForm.senderName" placeholder="请输入发货人姓名" />
        </el-form-item>
        <el-form-item label="手机号">
          <el-input v-model="addressForm.senderPhone" placeholder="请输入手机号" />
        </el-form-item>
...
const addresses = ref<FarmerAddress[]>([])
...
const fetchAddressList = async () => {
  loading.value = true
  try {
    const res: any = await request.get('/farmer/address')
    addresses.value = res || []
  } catch (error) {
...
const editAddress = (addr: FarmerAddress) => {
  isEdit.value = true
  currentId.value = addr.id || null
  addressForm.senderName = addr.senderName
  addressForm.senderPhone = addr.senderPhone
  // 将文本名称转换为编码
  addressForm.region = [
...
const deleteAddress = async (id: number | undefined) => {
  ...
  ElMessageBox.confirm('确定要删除该地址吗？', '提示', {
    ...
    try {
      await request.delete(`/farmer/address/${id}`)
...
const setDefault = async (id: number | undefined) => {
  ...
  try {
    await request.put(`/farmer/address/${id}/default`)
...
const saveAddress = async () => {
  // 表单验证
  if (!addressForm.senderName.trim()) {
    ElMessage.warning('请输入发货人姓名')
    return
  }
  if (!addressForm.senderPhone.trim()) {
  ...
  try {
    ...
    const params = {
      ...
      senderPhone: addressForm.senderPhone,
    ...
    if (isEdit.value && currentId.value) {
      await request.put(`/farmer/address/${currentId.value}`, params)
      ElMessage.success('修改成功')
    } else {
      await request.post('/farmer/address', params)
...
const resetForm = () => {
  addressForm.senderName = ''
  addressForm.senderPhone = ''
  addressForm.region = []
